...
class ConfigLoader:
    ...
    def process_zone_line(self, line: str) -> Zone:
        ...
        if ":" in rest:
            raise e.ZoneFormatError(
                f"Zone name can not contain ':' symbol: {line}"
                )
        ...
        try:
            ...
            if drone_capacity < 0:
                raise e.ZoneValueError(
                        (
                            f"Capacity of the Zone '{name}' can't be < 0: "
                            f"{drone_capacity}"
                        )
                    )
        except ValueError:
                ...
                if line.startswith("nb_drone"):
                    ...
                    try:
                        ...
                        if val <= 0 or val > 100:
                            raise e.DroneValueError(
                                (
                                    "nb_drones val should be in range "
                                    "0 to 100: "
                                    f"{val}"
                                )
                                )
            ...
            for line in lines:
                if line.startswith("connection"):
                    ...
                    try:
                        ...
                        raise e.ZoneFormatError(
                            (
                                "Invalid connection val, "
                                f"check your conf file: {line}"
                            )
                            )
                    ...
                    try:
                        ...
                        raise ConnectionError(
                            "Invalud max_link_capacity value"
                        )
