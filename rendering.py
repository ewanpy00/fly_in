...
class Renderer:
    ...
    def draw_connections(
        ...
        for zone in map_obj.zones:
            start_pos = self._screen_coords(zone.x, zone.y, min_max)
            for neighbor in zone.connections:
                end_pos = self._screen_coords(neighbor.x, neighbor.y, min_max)
                pygame.draw.line(
                    screen,
                    (100, 100, 100),
                    start_pos,
    ...
    def draw_zones(
        ...
        map_obj: Map,
        min_max: Tuple[float, float, float, float],
        mouse_p: Tuple[int, int],
    ) -> Optional[Zone]:
        hovered_zone: Optional[Zone] = None
        for zone in map_obj.zones:
            if zone.is_visible is True:
                zone_p = self._screen_coords(zone.x, zone.y, min_max)
    ...
    def draw_tooltip(
        ...
        pos = self._screen_coords(zone.x, zone.y, min_max)
