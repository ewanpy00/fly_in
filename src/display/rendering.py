import pygame

def draw_connections(screen, map_obj, min_max):
    from src.display.display import get_screen_coords
    """Рисуем линии между зонами"""
    for zone in map_obj.zones:
        start_pos = get_screen_coords(zone.x, zone.y, min_max)
        for neighbor in zone.connections:
            end_pos = get_screen_coords(neighbor.x, neighbor.y, min_max)
            pygame.draw.line(screen, (100, 100, 100), start_pos, end_pos, 2)

def draw_zones(screen, map_obj, min_max, mouse_pos):
    from src.display.display import get_screen_coords
    hovered_zone = None
    for zone in map_obj.zones:
        zone_pos = get_screen_coords(zone.x, zone.y, min_max)
        dist = ((zone_pos[0] - mouse_pos[0])**2 + (zone_pos[1] - mouse_pos[1])**2)**0.5
        
        color = getattr(zone, 'color', "white")
        radius = 15
        
        if dist < radius:
            draw_color = pygame.Color("white")
            pygame.draw.circle(screen, draw_color, zone_pos, radius + 3)
            hovered_zone = zone
        else:
            try:
                draw_color = pygame.Color(color)
            except ValueError:
                draw_color = pygame.Color("white")
            pygame.draw.circle(screen, draw_color, zone_pos, radius)
            
    return hovered_zone

def draw_tooltip(screen, zone, min_max):
    from src.display.display import get_screen_coords
    pos = get_screen_coords(zone.x, zone.y, min_max)
    font = pygame.font.SysFont("Arial", 18, bold=True)

    # Создаем текст
    text_surf = font.render(zone.name, True, (255, 255, 255))
    text_rect = text_surf.get_rect(center=(pos[0], pos[1] - 35))

    # Рисуем подложку (фон) чуть больше текста
    bg_rect = text_rect.inflate(14, 8)
    pygame.draw.rect(screen, (40, 40, 40), bg_rect, border_radius=5)
    # Тонкая рамка вокруг подложки
    pygame.draw.rect(screen, (200, 200, 200), bg_rect, width=1, border_radius=5)
    
    screen.blit(text_surf, text_rect)

def draw_ui_counter(screen, turn_counter, font):
    text_str = f"Total Turns: {turn_counter}"
    turn_surf = font.render(text_str, True, (0, 255, 0))
    
    text_pos = (25, 20)
    
    shadow_surf = font.render(text_str, True, (0, 50, 0))
    screen.blit(shadow_surf, (text_pos[0] + 2, text_pos[1] + 2))
    
    screen.blit(turn_surf, text_pos)